...
The manual-SSH-unlock decision (§9, Q-N3) assumes SSH to the cell works *at the moment it is needed*. That assumption is weaker than it looks, and the drill has to be designed around the weakness rather than around convenience.

**The standing constraint** (operator, 2026-07-18, tasks #2195–#2197): the cell is on a USB Wi-Fi dongle at **-83 dBm with ~40% packet loss to its own gateway**, the operator has no physical access to the ethernet, and this will not be fixed in the foreseeable window. Treat it as permanent, not as a pending fix.

**Why it matters here specifically**: a power cut takes down the cell *and* the network equipment. On recovery the dongle must re-associate before the initramfs SSH server is reachable at all — so "the operator connects and unlocks" is a probabilistic claim on a lossy link, not a given. The failure mode is not a slow unlock; it is **the operator being unable to reach the box to unlock it, leaving the cell dark until they attend physically** — the worst case §9 accepted, made materially more likely by the link.

This does **not** reopen Q-N3. The security reasoning behind it — stolen hardware must not carry its own key — is entirely unaffected by link quality, and both rejected alternatives (USB key in the chassis, clevis+tang) fail for reasons the network cannot fix. What it changes is what the drill must prove:

- **Run the drill over the real degraded link**, from off-box, exactly as a 3 a.m. recovery would happen. A drill run from a shell already on the cell proves nothing about the only scenario that matters.
- **Measure time-to-reachable** from cold boot: how long until initramfs SSH answers, across several attempts, so the operator knows whether to expect 30 seconds or 10 minutes.
- **Tune the initramfs network timeout upward** until re-association completes reliably. Patience in the boot path is cheap; a failed unlock is not.
- **Flakiness is answered with retry patience, never with a key in the chassis.** Stated here explicitly so a frustrating drill does not become the argument for undoing the security decision.
